...
import type { ExportJob, QueueItem } from "../../lib/export/types";
...
export default function ExportQueue({ queue, onRemove, onStarted }: ExportQueueProps) {
  ...
  const [completedJobIds, setCompletedJobIds] = useState<Record<string, string>>({}); // localId -> jobId
  ...
  const handleStatusChange = useCallback((localId: string, job: ExportJob | null) => {
    setCompletedJobIds((prev) => {
      if (job?.status === "complete") {
        if (prev[localId] === job.job_id) return prev;
        return { ...prev, [localId]: job.job_id };
      }
      if (prev[localId]) {
  ...
  const readyJobIds = useMemo(() => Object.values(completedJobIds), [completedJobIds]);

  const downloadAllAsZip = useCallback(async () => {
    const res = await fetch(`${API_BASE}/api/export/download-batch`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(readyJobIds),
    });
    if (!res.ok) return;
    ...
    a.href = url;
    a.download = "forgeos-exports.zip";
    a.click();
    ...
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {readyJobIds.length >= 2 && (
            <button
              type="button"
              onClick={downloadAllAsZip}
              className="rounded-md border border-emerald-700 px-3 py-1.5 text-xs font-medium text-emerald-300 hover:bg-emerald-900/30"
            >
              Download all ({readyJobIds.length}) as ZIP
            </button>
          )}
          <button
            type="button"
      ...
      <ul className="flex flex-col gap-2">
        {queue.map((item) => (
          <QueueRow key={item.localId} item={item} onRemove={() => onRemove(item.localId)} onStatusChange={handleStatusChange} />
        ))}
      </ul>
    </div>
  );
...
function QueueRow({
  ...
  item: QueueItem;
  onRemove: () => void;
  onStatusChange: (localId: string, job: ExportJob | null) => void;
}) {
  ...
  useEffect(() => {
    onStatusChange(item.localId, job);
  }, [item.localId, job, onStatusChange]);
