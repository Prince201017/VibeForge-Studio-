...
def generate_container_queries(name: str, selector: str, breakpoints: List[dict],
                                container_name: str | None = None) -> str:
    """
    Container-query equivalent of generate_media_queries, for components whose
    animation should respond to their containing element's size rather than
    the viewport. Requires a `container-type: inline-size` ancestor.
    """
    blocks = []
    for bp in breakpoints or []:
        overrides = bp.get("overrides") or {}
        ...
        if not decls:
            continue
        max_width = bp.get("maxWidth")
        container_clause = f"{container_name} " if container_name else ""
        query = f"@container {container_clause}(max-width: {max_width}px)" if max_width else f"@container {container_clause}(min-width: 1024px)"
        blocks.append(f"{query} {{\n  {selector} {{\n    " + "\n    ".join(decls) + "\n  }\n}")
    return "\n\n".join(blocks)


def generate_full_css(config: dict, vendor_prefixes: bool = True, minify: bool = False,
                       use_container_queries: bool = False) -> str:
    name = config["name"]
    ...
    if use_container_queries:
        media = generate_container_queries(name, selector, config.get("breakpoints", []))
    else:
