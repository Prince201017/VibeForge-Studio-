...
from models.schemas import (
    ...
    Keyframe, Layer, ProjectData, RenderOptions, ShapeType, CodeOptions,
)
...
class TestCodeExport:
    ...
    @pytest.mark.asyncio
    async def test_html_minify_tree_shakes_css(self, engine, project):
        req = ExportRequest(project=project, format=ExportFormat.HTML, code=CodeOptions(
            minify=False, typescript=True, css_scope="inline", include_responsive=True, component_name="X",
        ))
        req_minified = req.model_copy(update={"code": req.code.model_copy(update={"minify": True})})
        job = await engine.start(req_minified)
        await engine._tasks[job.job_id]
        final = engine.get(job.job_id)
        assert final.status.value == "complete"
...
class TestTreeShaking:
    def test_drops_unreferenced_rules_and_keyframes(self):
        from services.optimization import tree_shake_css

        css = (
            ".used { animation: used-anim 1s; }\n"
            "@keyframes used-anim { 0% { opacity: 0; } 100% { opacity: 1; } }\n"
            ".unused { color: red; }\n"
            "@keyframes unused-anim { 0% { opacity: 0; } 100% { opacity: 1; } }\n"
        )
        result = tree_shake_css(css, '<div class="used"></div>')
        assert ".used" in result and "used-anim" in result
        assert ".unused" not in result
        assert "unused-anim" not in result
...
class TestApiRoutes:
    ...
    def test_download_batch_bundles_multiple_completed_exports(self, project):
        import io
        import time
        import zipfile
        ...
        client = TestClient(app)
        job_ids = []
        for fmt in (ExportFormat.CSS, ExportFormat.LOTTIE):
            resp = client.post("/api/export/start", json=json.loads(
                ExportRequest(project=project, format=fmt).model_dump_json()
            ))
            job_ids.append(resp.json()["job_id"])

        for _ in range(50):
            statuses = [client.get(f"/api/export/progress/{jid}").json()["status"] for jid in job_ids]
            if all(s in ("complete", "failed") for s in statuses):
                break
            time.sleep(0.1)

        resp = client.post("/api/export/download-batch", json=job_ids)
        assert resp.status_code == 200
        zf = zipfile.ZipFile(io.BytesIO(resp.content))
        assert len(zf.namelist()) == 2

    def test_download_batch_rejects_empty_list(self):
        from fastapi.testclient import TestClient
        from main import app

        client = TestClient(app)
        resp = client.post("/api/export/download-batch", json=[])
        assert resp.status_code == 400

    def test_share_rejects_local_only_export(self, project):
        import time
        ...
        for _ in range(50):
            if client.get(f"/api/export/progress/{job_id}").json()["status"] == "complete":
                break
            time.sleep(0.1)
        resp = client.post(f"/api/export/share/{job_id}")
        assert resp.status_code == 400


class TestStorageExport:
    @pytest.mark.asyncio
    async def test_shareable_link_rejects_local_destination(self):
        from models.schemas import StorageTarget
        from services.storage_export import UploadResult, generate_shareable_link

        with pytest.raises(ValueError, match="local"):
            await generate_shareable_link(UploadResult(url="/x", public=False), StorageTarget(destination="local"))

    @pytest.mark.asyncio
    async def test_s3_shareable_link_requires_provider_ref(self):
        from models.schemas import StorageTarget
        from services.storage_export import UploadResult, generate_shareable_link

        with pytest.raises(ValueError, match="provider_ref"):
            await generate_shareable_link(UploadResult(url="x", public=False), StorageTarget(destination="s3"))

    @pytest.mark.asyncio
    async def test_s3_shareable_link_presigns_with_expiry(self):
        from unittest.mock import MagicMock, patch

        from models.schemas import StorageTarget
        from services.storage_export import UploadResult, generate_shareable_link

        result_in = UploadResult(url="https://bucket.s3.amazonaws.com/x.mp4", public=False,
                                  provider_ref={"bucket": "bucket", "key": "exports/x.mp4"})
        with patch("boto3.client") as mock_client:
            mock_s3 = MagicMock()
            mock_s3.generate_presigned_url.return_value = "https://signed.example/x.mp4?sig=abc"
            mock_client.return_value = mock_s3
            result = await generate_shareable_link(result_in, StorageTarget(destination="s3"), expires_in_sec=600)
        assert result.url == "https://signed.example/x.mp4?sig=abc"
        assert result.expires_at is not None

    @pytest.mark.asyncio
    async def test_gdrive_private_share_raises_documented_api_constraint(self):
        from models.schemas import StorageTarget
        from services.storage_export import UploadResult, generate_shareable_link

        with pytest.raises(ValueError, match="expirationTime"):
            await generate_shareable_link(
                UploadResult(url="x", public=False, provider_ref={"file_id": "abc"}),
                StorageTarget(destination="gdrive", public=False),
            )

    @pytest.mark.asyncio
    async def test_vercel_blob_share_is_passthrough(self):
        from models.schemas import StorageTarget
        from services.storage_export import UploadResult, generate_shareable_link

        result_in = UploadResult(url="https://blob.vercel-storage.com/x.mp4", public=True)
        result = await generate_shareable_link(result_in, StorageTarget(destination="vercel_blob", public=True))
        assert result.url == result_in.url
