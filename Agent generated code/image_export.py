...
async def export_image_sequence(request: ExportRequest, progress_cb: ProgressCallback) -> ExportResult:
    ...
    for i in range(total_frames):
        ...
        await loop.run_in_executor(None, _save_single_frame, frame, path, fmt, request)
...
def _save_single_frame(frame: Image.Image, path: Path, fmt: ExportFormat, request: ExportRequest) -> None:
    ...
    if fmt == ExportFormat.PNG_SEQUENCE or fmt == ExportFormat.APNG:
        ...
        _save_exr(frame, path)
    else:
        raise ValueError(f"{fmt} is not an image sequence format")


def _save_exr(frame: Image.Image, path: Path) -> None:
    if not _HAS_EXR:
        raise RuntimeError(
