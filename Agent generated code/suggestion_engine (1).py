...
class SuggestionEngine:
    ...
    async def suggest(self, request: SuggestRequest, layer_context: dict | None = None) -> tuple[SuggestResponse, list[str]]:
        ...
        if request.suggestionType == "color_harmony":
            ...
            # Model-backed suggestion types are the ones worth batching —
            # windows a burst of near-simultaneous requests (e.g. a UI
            # that fires a suggest() call per keystroke/drag-tick) into
            # one upstream call instead of N.
            suggestions = await self.batch_manager.windowed_batch(
                group_key=f"suggest:{request.suggestionType}",
                item=(request.suggestionType, layer_context),
                executor=self._execute_suggestion_batch,
            )
    ...
    async def _execute_suggestion_batch(self, items: list[tuple[str, dict]]) -> list[list[Suggestion]]:
        """Runs every item in the batch concurrently and returns results in the same order."""
        import asyncio

        results = await asyncio.gather(
            *(self._suggest_via_model(suggestion_type, context) for suggestion_type, context in items)
        )
        return list(results)
