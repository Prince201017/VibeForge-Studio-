...
class Project(Base):
    ...
    storage_quota_bytes: Mapped[int] = mapped_column(BigInteger, server_default="5368709120")
...
class Operation(Base):
    """
    [Claude.DB] Hash-partitioned by project_id as of migration 005. Primary
    key is (id, project_id) because Postgres requires partition keys to be
    part of any unique/PK constraint on a partitioned table. As a result
    `parent_operation_id` is a plain column, not a foreign key - Postgres
    can't enforce a FK against a non-unique (id)-only reference on a
    partitioned table, so parent/child operation linkage is validated at
    the application layer (see services/db_service.py OperationService).
    """
    ...
    parent_operation_id: Mapped[Optional[uuid.UUID]] = mapped_column(PG_UUID(as_uuid=True))
...
class AuditLog(Base):
    """
    [Claude.DB] Range-partitioned by month on created_at as of migration 005.
    Primary key is (id, created_at) for the same partitioning-key reason
    documented on Operation above.
    """
