...
from utils.moderation import ModerationUnavailableError, get_moderation_service
...
class ReferenceAnalysisBlockedError(Exception):
    pass
...
class ReferenceAnalyzer:
    ...
    async def analyze(self, request: AnalyzeReferenceRequest) -> tuple[AnalyzeReferenceResponse, list[str]]:
        ...
        try:
            ...
            verdict = await moderation_service.moderate_image(request.imageUrl)
            if verdict.flagged:
                flagged_categories = [k for k, v in verdict.categories.items() if v]
                raise ReferenceAnalysisBlockedError(
                    f"Reference image flagged by moderation API: {', '.join(flagged_categories)}"
                )
        except ModerationUnavailableError as exc:
            logger.info("Image moderation unavailable, proceeding without it: %s", exc)
            warnings.append("Real-time image moderation was unavailable for this reference image.")
