...
describe("Select", () => {
  ...
  it("matches snapshot", () => {
    const { container } = render(<Select options={options} value={null} onChange={vi.fn()} placeholder="Choose one" />);
    expect(container).toMatchSnapshot();
  });

  it("has no a11y violations", async () => {
    const result = render(<Select options={options} value={null} onChange={vi.fn()} placeholder="Choose one" />);
