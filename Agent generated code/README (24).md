...
| Unit tests | 100+ | **216 test cases** across 48 test files |
| Snapshot tests | Required | **48/48** components covered |
| Accessibility tests (axe-core) | Required | **48/48** components covered via `jest-axe` |
...
| Accessibility | WCAG 2.1 AA | ✅ semantic roles, keyboard nav, checklist in `ACCESSIBILITY.md` |
...
- **Storybook/Vitest were not run in this sandbox** (no network to install deps) — configs are written and should work as-is, but haven't been executed end-to-end here. Run `npm install && npm test` before merging.
...
- **Screen-reader manual testing** wasn't performed (no screen reader available in this sandbox) — `jest-axe` covers the same class of issues programmatically; see `ACCESSIBILITY.md` for the one item worth a manual pass before shipping.
- **Focus trapping** in Modal/Drawer/ConfirmDialog moves focus in on open and closes on Escape, but doesn't cycle Tab back to the first element yet — noted in `ACCESSIBILITY.md`.
...
- A couple of components (`Select`, `Table`) intentionally avoid a virtualization library dependency — they use a hand-rolled windowing technique adequate for the stated 10K-item target but less robust than `react-window` under extreme edge cases (e.g. widely varying row heights).
