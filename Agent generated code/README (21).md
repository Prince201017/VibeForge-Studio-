...
and validated Pydantic schemas. All Python files pass `ast.parse`;
dependency-free modules import-test cleanly; the batching logic, color
math, and TypeScript inference were exercised live in this sandbox with
real assertions, not just eyeballed.

What it has **not** been is run against live provider API keys or real
image/3D-model files (no network access in this sandbox), so treat the
OpenAI/Anthropic/Gemini/Stability/Whisper/Moderation/vectorization/
trimesh code paths as unverified against live services until you plug in
credentials and test data. `scikit-image` and `trimesh` are new
dependencies added for this pass — installing them pulls in `numpy`,
`scipy`, and (for trimesh) a fairly large dependency tree; budget for
that in your build image.
